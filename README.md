...
- **Per-host advanced overrides** — `listen_host`, `mtu`, `tun_device`, `punchy`, `punchy_respond`, `unsafe_routes` opt-in per host without touching the network default. Host records are editable via UI/API after creation (`PATCH /api/v1/hosts/{id}`).
- **Gateways to non-Nebula networks** — `unsafe_networks` on a host signs the prefixes it may route for into its certificate, the provider half of another host's `unsafe_routes`. Nebula authorizes routing on the certificate, so without it the gateway silently refuses to route and peers drop the traffic before any firewall rule runs. Editing it is a certificate-bound change, so the gateway re-enrolls on its next poll and the new certificate carries the prefix. See [docs/agent.md](docs/agent.md#routing-to-a-non-nebula-network).
