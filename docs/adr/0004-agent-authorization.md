...
- **Force-rotate.** `POST /api/v1/hosts/{id}/rotate-cert?new_key=true|false`. Same-key rotation re-signs immediately; new-key rotation sets a `pending_rekey` flag on the host row; next poll response carries `{rekey_required: true, enrollment_token: <new>}`; agent generates a fresh keypair, calls `/api/v1/enroll`, swaps files atomically, resumes polling.
...
- `internal/api/updates.go`, `internal/api/hosts.go`, `internal/agent/poller.go`, `internal/store` and one new migration touched. No CA-side changes (orthogonal to ADR 0002 / 0003).
- README + `docs/agent.md` add "Agent authorization" sections explaining the three modes and the migration window.
