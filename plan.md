# Plan: Nebula Mesh Management Platform — MVP
...
Создать self-hosted платформу для управления Nebula mesh-сетью: management server с REST API и агент для хостов. MVP покрывает: PKI engine, token-based enrollment, config generation, pull-based agent для Linux/macOS.
...
Storage — SQLite через `modernc.org/sqlite` (pure Go, без CGO). API — стандартная библиотека `net/http` + chi router. Минимум зависимостей.
